...
export default async function CalendarPage({
  ...
  const allTrades: CalendarTrade[] = await getTrades()
  ...
  allTrades.forEach((trade) => {
    if (!trade.created_at || trade.status === 'OPEN') {
      return
    }
  ...
  const monthlyPnL = dailyValues.reduce(
    (acc, curr) => acc + curr.pnl,
    0
  )

  const monthlyTradesCount = dailyValues.reduce(
    (acc, curr) => acc + curr.tradesCount,
    0
  )

  const totalTradingDays = Object.keys(dailyStats).length

  const winningDays = dailyValues.filter(
    (day) => day.pnl > 0
  ).length

  const losingDays = dailyValues.filter(
    (day) => day.pnl < 0
  ).length
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Monthly overview of your daily trading results and
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {monthlyTradesCount} closed trades
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {totalTradingDays
                ? `${(
                    (winningDays / totalTradingDays) *
                    100
                  ).toFixed(0)}% day win rate`
                : '0%'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {totalTradingDays
                ? `${(
                    (losingDays / totalTradingDays) *
                    100
                  ).toFixed(0)}% day loss rate`
                : '0%'}
            </p>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-4 sm:p-6">
          ...
          <div className="grid grid-cols-7 gap-1 sm:gap-2">
            ...
            {Array.from({
              ...
              <div
                key={`empty-${i}`}
                className="h-24 sm:h-28 rounded-md bg-muted/20 border border-transparent"
              />
            ))}
